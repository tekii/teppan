...
#   secret-tool store --label='tekii/teppan gh PAT' service gh-token account tekii-teppan
# Manage it in Seahorse: Passwords -> Login -> "tekii/teppan gh PAT".

GH_TOKEN="$(secret-tool lookup service gh-token account tekii-teppan 2>/dev/null || true)"
if [ -z "${GH_TOKEN}" ]; then
  echo "code-with-gh-token: no token found in the keyring" >&2
  echo "  (service=gh-token account=tekii-teppan) -- is the keyring unlocked?" >&2
  echo "  store it with:" >&2
  echo "    secret-tool store --label='tekii/teppan gh PAT' service gh-token account tekii-teppan" >&2
  exit 1
fi
export GH_TOKEN
exec code "$@"
