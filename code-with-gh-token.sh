...
#   secret-tool store --label='tekii/www gh PAT' service gh-token account tekii-www
# Manage it in Seahorse: Passwords -> Login -> "tekii/www gh PAT".
...
if [ -z "${GH_TOKEN}" ]; then
  ...
  echo "    secret-tool store --label='tekii/www gh PAT' service gh-token account tekii-www" >&2
  exit 1
fi
export GH_TOKEN
exec code "$@"
