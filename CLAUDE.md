...
- **`header svg:focus, amp-sidebar svg:focus { outline: none; }`**
  ...
  Visible) violation regardless of AMP. Cross-reference the HTML role's note
  on `role="button"`/`tabindex` SVG icons in `layout.html`: any focusable
  element needs *some* visible focus style.
- **AMP coupling left behind during AMP removal** (tracked separately, see
  `NOTES.md`/AMP-removal effort):
